...
SupaCodex publishes its macOS Homebrew cask through a separate tap repository:

- Tap repo: `wygoralves/homebrew-tap`
- Published file: `Casks/supacodex.rb`
...
3. The main SupaCodex repo has a `HOMEBREW_TAP_TOKEN` secret with write access to the tap repository.
...
1. Checks out the released tag from the SupaCodex repo.
2. Checks out the tap repository into a sibling working directory.
3. Runs `node scripts/generate-homebrew-cask.mjs <tag> --output homebrew-tap/Casks/supacodex.rb`.
