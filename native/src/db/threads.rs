...
pub fn upsert_imported_codex_thread(
    db: &Database,
    workspace_id: &str,
    ...
    let imported_profile_id = codex_profile_id_from_metadata(metadata);
    let existing = list_threads_by_engine_thread_id(db, "codex", engine_thread_id)?
        .into_iter()
        .find(|thread| {
            thread.workspace_id == workspace_id
                && codex_profile_id_from_metadata_value(thread.engine_metadata.as_ref())
                    == imported_profile_id
        })
        .or_else(|| {
            list_threads_by_engine_thread_id(db, "codex", engine_thread_id)
                .ok()
                .and_then(|threads| {
                    threads
                        .into_iter()
    ...
    if let Some(existing) = existing {
        let merged_metadata = merge_engine_metadata(existing.engine_metadata.as_ref(), metadata);
        let next_title = if thread_manual_title_locked(existing.engine_metadata.as_ref()) {
        ...
        conn.execute(
            "UPDATE threads
             SET model_id = ?1,
                 ...
                 archived_at = CASE WHEN ?4 THEN COALESCE(archived_at, datetime('now')) ELSE NULL END,
                 last_activity_at = ?5
             WHERE id = ?6",
            params![
                model_id,
        ...
        .context("failed to update imported codex thread")?;

        return get_thread(db, &existing.id)?
            .ok_or_else(|| anyhow::anyhow!("thread not found after codex import update"));
    }
    ...
    conn.execute(
        "INSERT INTO threads (
            ...
            engine_metadata_json, title, status, archived_at, created_at, last_activity_at
         ) VALUES (?1, ?2, NULL, 'codex', ?3, ?4, ?5, ?6, 'idle',
                   CASE WHEN ?7 THEN ?9 ELSE NULL END, ?8, ?9)",
        params![
            id,
            workspace_id,
            model_id,
    ...
    .context("failed to insert imported codex thread")?;

    get_thread(db, &id)?.context("thread not found after codex import insert")
}
...
fn merge_engine_metadata(existing: Option<&Value>, imported: &Value) -> Value {
    let mut output = existing.cloned().unwrap_or_else(|| serde_json::json!({}));
    if !output.is_object() {
        output = serde_json::json!({});
    }

    if let (Some(output), Some(imported)) = (output.as_object_mut(), imported.as_object()) {
...
fn codex_profile_id_from_metadata(metadata: &Value) -> Option<&str> {
    metadata
        .get("codexProfileId")
...
fn codex_profile_id_from_metadata_value(metadata: Option<&Value>) -> Option<&str> {
    metadata
...
mod tests {
    ...
    #[test]
    fn upsert_imported_codex_thread_creates_and_updates_engine_backed_thread() {
        let db = test_db();
        let root = std::env::temp_dir().join(format!("supacodex-workspace-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).expect("failed to create temp workspace root");
        let workspace =
        ...
        let first = upsert_imported_codex_thread(
            &db,
            &workspace.id,
            "codex-thread-cli",
            "gpt-5.3-codex",
            ...
            &json!({
                ...
                "codexRemoteUpdatedAt": "2026-05-24T10:05:00Z",
            }),
        )
        .unwrap();
        let updated = upsert_imported_codex_thread(
            &db,
            &workspace.id,
            "codex-thread-cli",
            "gpt-5.3-codex",
            ...
            &json!({
                "codexProfileId": "default",
                "codexRemoteUpdatedAt": "2026-05-24T10:10:00Z",
            }),
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            Some("codex-thread-cli")
        );
        assert_eq!(updated.title, "CLI title updated");
        assert_eq!(updated.last_activity_at, "2026-05-24T10:10:00Z");

        let listed = list_threads_for_workspace(&db, &workspace.id).unwrap();
        assert!(listed.iter().any(|thread| thread.id == first.id));
    }
}
